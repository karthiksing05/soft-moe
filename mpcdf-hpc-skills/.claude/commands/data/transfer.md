# /data:transfer — Move data to/from/between MPCDF systems
...
1. If the transfer involves a cluster and `SSH_ALIAS` is not set, read
   `skills/cluster/resolve.md`. → `CLUSTER`, `SSH_ALIAS`.
2. Read `skills/data/transfer.md` and follow every step exactly. It picks the
   method (rsync/scp via gate, Globus/DataHub, DataShare, Nexus-S3, archive)
   from the size and purpose, then runs and verifies the transfer.
