...
| [SYNTHETIC_SYNTAX.md](SYNTHETIC_SYNTAX.md) | **impossible** synthetic transforms (no priors to fall back on) | no | **12.1% → 100% (SFT) / 99.6% (EM)** exact-match | **maximal (SFT ×12.1, EM ×4.6, swap acc → 0%)** |
...
| [SYNTHETIC_SYNTAX.md](SYNTHETIC_SYNTAX.md#part-2--incrementally-adding-a-genuinely-novel-transform) | does token-only still work when the new persona is *novel computation*, not a latent style? | **no free lunch**: token-only keeps the base (86.7%) but only partially learns the novel transform (41.7%, plateaus); full-SFT/EM reach 100% but **catastrophically forget** the base (7.1%/0%) — the token selects, it doesn't store new computation |
| [SYNTHETIC_SYNTAX.md](SYNTHETIC_SYNTAX.md#part-3--can-we-make-em-incremental-and-forgetting-robust-cyclic-ab-schedules) | can a cyclic A/B schedule get token-only retention **+** full-SFT performance? | **order beats granularity**: A-first EM forgets like full-SFT (base 15%); reordering to **token-first** (same budget, no replay) recovers base **15%→59%** at new=100%; replay is the ceiling (93%) but needs base data |
